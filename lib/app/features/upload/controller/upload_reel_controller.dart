...
class UploadReelController extends BaseController<UploadReelUiState> {
  ...
  void pickFromGallery() async {
    ...
    if (pickedImage != null && pickedImage.path.isValidVideoExtension()) {
      ...
      showErrorMessage('Please, select a valid video file, only MP4 and MOV formats are allowed');
    }
  }

  void videoSelectedFromCamera(String videoFilePath) async {
    if(uiData.videoFilePath != null) {
      final videoFileData = await File(uiData.videoFilePath!).readAsBytes();
      updateState(uiData.copyWith(
      ...
      showErrorMessage('An error occurred while picking the reel');
    }
  }
