...
class ProfileController extends BaseController<ProfileUiData> {
  ...
  void onInit() {
    ...
    if (args.containsKey(USER_UUID_KEY)) {
      String userUuid = args[USER_UUID_KEY];
  ...
  void _loadUserDetails(String userUuid) async {
    try {
      ...
      updateState(uiData.copyWith(userData: userDetails, userUuid: userUuid, isAuthUser: authUserUid == userUuid));
    } catch (e) {
  ...
  void _loadCurrentUserDetails() async {
    try {
      final String authUserUid = await getAuthUserUidUseCase(const DefaultParams());
      final userDetails = await getUserDetailsUseCase(GetUserDetailsParams(authUserUid));
      updateState(uiData.copyWith(userData: userDetails, userUuid: authUserUid, isAuthUser: true));
    } catch (e) {
      showErrorMessage("An error occurred while loading user details");
    }
  }
